...
/**
 * Read files in comma separated value format. More information about this class
 ...
 * is the Microsoft Excel spreadsheet. A separate class must be use to read <a
 * href="http://ostermiller.org/utils/ExcelCSV.html">Excel CSV</a>.
 * 
...
public class CSVParser implements CSVParse {
	...
	public CSVParser(InputStream in) {
		this.inStream = in;
		this.lexer = new CSVLexer(new java.io.InputStreamReader(in));
	}

	public CSVParser(InputStream in, String charset)
			throws UnsupportedEncodingException {
		this.inStream = in;
		this.lexer = new CSVLexer(new java.io.InputStreamReader(in, charset));
	}
	...
	public String[] getLine() throws IOException {
		...
		while ((this.tokenCache = this.lexer.getNextToken()) != null
				&& (lineNumber == -1 || this.lexer.getLineNumber() == lineNumber)) {
			v.add(this.tokenCache);
	...
	/**
	 * Parse the given file for comma separated values and print the results to
	 * System.out.
	 * 
	 * @param args
	 *            First argument is the file name. System.in used if no filename
	 *            given.
	 * 
	 * @since ostermillerutils 1.00.00
	 */
	 ...
	 * <p>
	 * Only escaped backslashes and quotes will be recognized as escape
	 * sequences. The data will be treated as having no comments.
	 * 
	 * @param s
	...
	public static String[][] parse(String s, char delimiter)
			throws BadDelimiterException {
		try {
			return (new CSVParser(new StringReader(s), delimiter))
					.getAllValues();
		} catch (IOException x) {
			return null;
		}
	...
	/**
	 * Parse the comma delimited data from a string. Escaped backslashes and
	 * quotes will always recognized as escape sequences.
	 * 
	 * @param s
	 *            string with comma delimited data to parse.
	 * @param escapes
	 *            a list of additional characters that will represent escape
	 *            sequences.
	 * @param replacements
	 *            the list of replacement characters for those escape sequences.
	 * @param commentDelims
	 ...
	 * @since ostermillerutils 1.02.03
	 */
	public static String[][] parse(String s, String escapes,
			String replacements, String commentDelims) {
		try {
			return (new CSVParser(new StringReader(s), escapes, replacements,
					commentDelims)).getAllValues();
		} catch (IOException x) {
			return null;
		}
	}
	...
	/**
	 * Parse the delimited data from a string. Escaped backslashes and quotes
	 * will always recognized as escape sequences.
	 * 
	 * @param s
	 *            string with delimited data to parse.
	 * @param escapes
	 ...
	 * @since ostermillerutils 1.02.24
	 */
	public static String[][] parse(String s, char delimiter, String escapes,
			String replacements, String commentDelims)
			throws BadDelimiterException {
		try {
			return (new CSVParser(new StringReader(s), delimiter, escapes,
					replacements, commentDelims)).getAllValues();
		} catch (IOException x) {
			return null;
		}
	}
	...
	public static String[][] parse(Reader in, char delimiter)
			throws IOException, BadDelimiterException {
	...
	/**
	 * Parse the delimited data from a stream. Escaped backslashes and quotes
	 * will always recognized as escape sequences.
	 * 
	 * @param in
	 ...
	 * @since ostermillerutils 1.02.24
	 */
	public static String[][] parse(Reader in, char delimiter, String escapes,
			String replacements, String commentDelims) throws IOException,
			BadDelimiterException {
		return (new CSVParser(in, delimiter, escapes, replacements,
				commentDelims)).getAllValues();
	}

	/**
	 * Parse the comma delimited data from a stream. Escaped backslashes and
	 * quotes will always recognized as escape sequences.
	 * 
	 * @param in
	 *            Reader with comma delimited data to parse.
	 ...
	 * @since ostermillerutils 1.02.03
	 */
	public static String[][] parse(Reader in, String escapes,
			String replacements, String commentDelims) throws IOException {
		return (new CSVParser(in, escapes, replacements, commentDelims))
				.getAllValues();
	}
}
