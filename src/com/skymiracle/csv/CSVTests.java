...
/**
 * Regression test for CSV. More information about this class is available from
 * <a target="_top" href=
...
class CSVTests {
	public static void main(String[] args) {
		try {
			...
			csvOut.println(new String[] { "Boston", "San Francisco",
					"New York", "Chicago", "Houston", });
			csvOut.println();
			csvOut.println(""); // an empty value on a line by itself.
			csvOut.println(new String[] { "Two\nTokens", "On the\nSame Line" });
			csvOut
					.printlnComment("A two line comment\njust to see that it works");

			CSVParser shredder = new CSVParser(new StringReader(sw.toString()));
			shredder.setCommentStart("#;!");
			shredder.setEscapes("nrtf", "\n\r\t\f");
			compare(shredder.nextValue(), shredder.lastLineNumber(), "Boston",
					4);
			compare(shredder.nextValue(), shredder.lastLineNumber(),
					"San Francisco", 4);
			compare(shredder.nextValue(), shredder.lastLineNumber(),
					"New York", 4);
			compare(shredder.nextValue(), shredder.lastLineNumber(), "Chicago",
					4);
			compare(shredder.nextValue(), shredder.lastLineNumber(), "Houston",
					4);
			...
			shredder.changeQuote(',');
			shredder.changeDelimiter('"');
			compare("commas and quotes swapped", normalOutput, shredder
					.getAllValues());

			normalInput = "\"test\\\\\",test";
			normalOutput = new String[][] { { "test\\", "test" } };
			shredder = new CSVParser(new StringReader(normalInput));
			compare("backslash at end of quoted", normalOutput, shredder
					.getAllValues());
			...
			shredder = new CSVParser(new StringReader(normalInput));
			compare("white space around fields", normalOutput, shredder
					.getAllValues());
			...
			normalOutput = new String[][] { { "1", "to", "tre", "four",
					"five5", "sixsix" } };
			shredder = new CSVParser(new StringReader(normalInput));
			compare("various lengths", normalOutput, shredder.getAllValues());

			normalInput = "!comment\n !field1\n;comment\n ;field2\n#comment\n #field3";
			normalOutput = new String[][] { { "!field1" }, { ";field2" },
					{ "#field3" } };
			shredder = new CSVParser(new StringReader(normalInput));
			shredder.setCommentStart("#;!");
			compare("comment must start at beginning of line", normalOutput,
					shredder.getAllValues());
	...
	private static void compare(String value, int line, String expectedValue,
	...
	private static void compare(String testName, String[][] a, String[][] b)
		...
		if (a.length != b.length) {
			throw new Exception(testName + ": unexpected number of lines "
					+ a.length + " found " + b.length + " expected");
		}
		for (int i = 0; i < a.length; i++) {
			if (a[i].length != b[i].length) {
				throw new Exception(testName
						+ ": unexpected number of values in line: "
						+ b[i].length);
			}
