...
/**
 * Print values as a comma separated list that can be read by the Excel
 * spreadsheet. More information about this class is available from <a
 * target="_top" href=
...
public class ExcelCSVPrinter implements CSVPrint {
	 ...
	 * @since ostermillerutils 1.00.00
	 */
	public ExcelCSVPrinter(OutputStream out, String charset) throws UnsupportedEncodingException {
	...
	/**
	 * Create a printer that will print values to the given stream. The
	 * delimiter will be the comma, and the quote character will be double
	 * quotes.
	 * 
	 * @param out
	 *            stream to which to print.
	 ...
	 */
	public ExcelCSVPrinter(Writer out, char quote, char delimiter,
			boolean alwaysQuote, boolean autoFlush)
			throws BadDelimiterException, BadQuoteException {
		this.out = out;
		changeQuote(quote);
		changeDelimiter(delimiter);
	...
	/**
	 * Since ExcelCSV format does not support comments, this method will ignore
	 * the comment and start a new row.
	 * <p>
	 * This method never throws an I/O exception. The client may inquire as to
	 * whether any errors have occurred by invoking checkError(). If an I/O
	...
	private String escapeAndQuote(String value) {
		String s = StringHelper
				.replace(value, String.valueOf(this.quoteChar), String
						.valueOf(this.quoteChar)
						+ String.valueOf(this.quoteChar));
		return (new StringBuffer(2 + s.length())).append(this.quoteChar)
				.append(s).append(this.quoteChar).toString();
	}
	...
	public static void main(String[] args) {
		...
		try {
			...
			ExcelCSVPrinter p = new ExcelCSVPrinter(out, "UTF-8");
			p.print("unquo,ted");
