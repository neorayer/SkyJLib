...
/**
 * Tests reading files in comma separated value format with a fist line of
 * labels.
 * 
...
class LabeledCSVParserTests {
	...
	private static void test01() throws Exception {
		...
		LabeledCSVParser parse = new LabeledCSVParser(new CSVParser(
				new StringReader("")));
	...
	private static void test02() throws Exception {
		...
		LabeledCSVParser parse = new LabeledCSVParser(new CSVParser(
				new StringReader("FIELD01,FIELD02,FIELD03")));
		if (parse.getLine() != null) {
			throw new Exception("Expected null from getLine()");
		}
		String[] labels = parse.getLabels();
		if (labels.length != 3 || !"FIELD01".equals(labels[0])
				|| !"FIELD02".equals(labels[1]) || !"FIELD03".equals(labels[2])) {
	...
	private static void test03() throws Exception {
		...
		LabeledCSVParser parse = new LabeledCSVParser(
				new CSVParser(new StringReader("FIELD01,FIELD02,FIELD03\n"
						+ "9.23,\"FOO\",\"BAR\"\n" + "10.5,\"BAZ\",\"FOO2\"\n")));
		String[] labels = parse.getLabels();
		if (labels.length != 3 || !"FIELD01".equals(labels[0])
				|| !"FIELD02".equals(labels[1]) || !"FIELD03".equals(labels[2])) {
		...
		if (parse.getValueByLabel("FIELD01") != null) {
			throw new Exception(
					"Line not yet read, expected value of FIELD01 to be null.");
		}
		...
		if (line.length != 3 || !"9.23".equals(line[0])
				|| !"FOO".equals(line[1]) || !"BAR".equals(line[2])) {
		...
		if (line.length != 2 || !"BAZ".equals(line[0])
				|| !"FOO2".equals(line[1])) {
			throw new Exception("Didn't get expected line.");
		}
		try {
