 ...
 * {@link #getValueByLabel(String)} allow these labels to be discovered and used
 * while parsing CSV data. This class can also be used to conveniently ignore
 * field labels if they happen to be present in a CSV file and are not desired.
 * 
...
public class LabeledCSVParser implements CSVParse {
	...
	/**
	 * Hash of the labels (String) to column number (Integer). Set by setLabels.
	 * 
	 * @since ostermillerutils 1.03.00
	 */
	 ...
	 * <p>
	 * The last line of the values is saved and may be accessed by
	 * getValueByLabel().
	 * 
	...
	public int lastLineNumber() {
		...
		if (lineNum <= -1)
			return -1; // Nothing has been read yet
		if (lineNum == 1)
			return -1; // only labels have been read
	 ...
	 * <p>
	 * <b>Note:</b> The methods {@link #nextValue()} and
	 * {@link #getAllValues()} are incompatible with
	 * {@link #getValueByLabel(String)} because the former methods cause the
	 * offset of field values to shift and corrupt the internal buffer
	 * maintained by {@link #getLine}.
	 * 
	 ...
	 * <p>
	 * This method is not compatible with getValueByLabel(). Using this method
	 * will make getValueByLabel() throw an IllegalStateException for the rest
	 * of the line.
	 * 
	...
	/**
	 * Initialize the LabeledCSVParser.labels member and
	 * LabeledCSVParser.labelMap member.
	 * 
	...
	/**
	 * Get the index of the column having the given label. The
	 * {@link #getLine()} method returns an array of field values for a single
	 * record of data. This method returns the index of a member of that array
	 * based on the specified field name. The first field has the index 0.
	 * 
	 * @param label
	 *            The field name.
	 * @return The index of the field name, or -1 if the label does not exist.
	 * @deprecated may swallow an IOException while reading the labels - please
	 *             use getLabelIdx()
	 * 
	 * @since ostermillerutils 1.03.00
	 */
	...
	public int getLabelIdx(String label) throws IOException {
		...
		if (!this.labelMap.containsKey(label))
			return -1;
		return ((Integer) this.labelMap.get(label)).intValue();
	}
	 ...
	 *             if nextValue has been called as part of getting the last
	 *             line. nextValue is not compatible with this method.
	 * @return the value from the last line read or null if there is no such
	 *         value
	 * 
	 * @since ostermillerutils 1.03.00
	 */
	public String getValueByLabel(String label) throws IllegalStateException {
		if (this.nextValueLine == getLastLineNumber())
			throw new IllegalStateException(
					"nextValue() was used to get values from this line.");
		...
		try {
			...
			// Can't happen here because the labels have been read before the
			// first line.
