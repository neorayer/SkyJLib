...
public class ExifDescriptor extends TagDescriptor {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 8214468119594483029L;
	/**
	...
	private String getThumbnailDescription() throws MetadataException {
		if (!this._directory.containsTag(ExifDirectory.TAG_THUMBNAIL_DATA))
			return null;
		int[] thumbnailBytes = this._directory
	...
	private String getIsoEquivalentDescription() throws MetadataException {
		if (!this._directory.containsTag(ExifDirectory.TAG_ISO_EQUIVALENT))
			return null;
		int isoEquiv = this._directory.getInt(ExifDirectory.TAG_ISO_EQUIVALENT);
		if (isoEquiv < 50) {
			isoEquiv *= 200;
		}
	...
	private String getReferenceBlackWhiteDescription() throws MetadataException {
		if (!this._directory
				.containsTag(ExifDirectory.TAG_REFERENCE_BLACK_WHITE))
			return null;
	...
	private String getExifVersionDescription() throws MetadataException {
		if (!this._directory.containsTag(ExifDirectory.TAG_EXIF_VERSION))
			return null;
		int[] ints = this._directory
				.getIntArray(ExifDirectory.TAG_EXIF_VERSION);
	...
	private String getFlashPixVersionDescription() throws MetadataException {
		if (!this._directory.containsTag(ExifDirectory.TAG_FLASHPIX_VERSION))
			return null;
		int[] ints = this._directory
				.getIntArray(ExifDirectory.TAG_FLASHPIX_VERSION);
		return ExifDescriptor.convertBytesToVersionString(ints);
	}
	...
	private String getSceneTypeDescription() throws MetadataException {
		if (!this._directory.containsTag(ExifDirectory.TAG_SCENE_TYPE))
			return null;
		int sceneType = this._directory.getInt(ExifDirectory.TAG_SCENE_TYPE);
		if (sceneType == 1) {
	...
	private String getFileSourceDescription() throws MetadataException {
		if (!this._directory.containsTag(ExifDirectory.TAG_FILE_SOURCE))
			return null;
		int fileSource = this._directory.getInt(ExifDirectory.TAG_FILE_SOURCE);
		if (fileSource == 3) {
	...
	private String getExposureBiasDescription() throws MetadataException {
		if (!this._directory.containsTag(ExifDirectory.TAG_EXPOSURE_BIAS))
			return null;
		Rational exposureBias = this._directory
	...
	private String getMaxApertureValueDescription() throws MetadataException {
		if (!this._directory.containsTag(ExifDirectory.TAG_MAX_APERTURE))
			return null;
		double apertureApex = this._directory
				.getDouble(ExifDirectory.TAG_MAX_APERTURE);
	...
	private String getApertureValueDescription() throws MetadataException {
		if (!this._directory.containsTag(ExifDirectory.TAG_APERTURE))
			return null;
		double apertureApex = this._directory
				.getDouble(ExifDirectory.TAG_APERTURE);
		double rootTwo = Math.sqrt(2);
		double fStop = Math.pow(rootTwo, apertureApex);
		java.text.DecimalFormat formatter = new DecimalFormat("0.#");
		return "F" + formatter.format(fStop);
	...
	private String getExposureProgramDescription() throws MetadataException {
		...
		// '6' program action(high-speed program), '7' portrait mode, '8'
		// landscape mode.
		...
		default:
			return "Unknown program ("
					+ this._directory
							.getInt(ExifDirectory.TAG_EXPOSURE_PROGRAM) + ")";
		}
	}
	...
	private String getPlanarConfigurationDescription() throws MetadataException {
		if (!this._directory
				.containsTag(ExifDirectory.TAG_PLANAR_CONFIGURATION))
			return null;
		...
		switch (this._directory.getInt(ExifDirectory.TAG_PLANAR_CONFIGURATION)) {
		case 1:
	...
	private String getSamplesPerPixelDescription() {
		if (!this._directory.containsTag(ExifDirectory.TAG_SAMPLES_PER_PIXEL))
			return null;
		return this._directory.getString(ExifDirectory.TAG_SAMPLES_PER_PIXEL)
				+ " samples/pixel";
	}

	private String getRowsPerStripDescription() {
		if (!this._directory.containsTag(ExifDirectory.TAG_ROWS_PER_STRIP))
			return null;
		return this._directory.getString(ExifDirectory.TAG_ROWS_PER_STRIP)
				+ " rows/strip";
	}

	private String getStripByteCountsDescription() {
		if (!this._directory.containsTag(ExifDirectory.TAG_STRIP_BYTE_COUNTS))
			return null;
		return this._directory.getString(ExifDirectory.TAG_STRIP_BYTE_COUNTS)
				+ " bytes";
	}
	...
	private String getCompressionDescription() throws MetadataException {
		if (!this._directory.containsTag(ExifDirectory.TAG_COMPRESSION))
			return null;
		// '1' means no compression, '6' means JPEG compression.
		switch (this._directory.getInt(ExifDirectory.TAG_COMPRESSION)) {
		case 1:
	...
	private String getBitsPerSampleDescription() {
		if (!this._directory.containsTag(ExifDirectory.TAG_BITS_PER_SAMPLE))
			return null;
		return this._directory.getString(ExifDirectory.TAG_BITS_PER_SAMPLE)
				+ " bits/component/pixel";
	}

	private String getThumbnailImageWidthDescription() {
		if (!this._directory
				.containsTag(ExifDirectory.TAG_THUMBNAIL_IMAGE_WIDTH))
			return null;
		return this._directory
				.getString(ExifDirectory.TAG_THUMBNAIL_IMAGE_WIDTH)
				+ " pixels";
	}

	private String getThumbnailImageHeightDescription() {
		if (!this._directory
				.containsTag(ExifDirectory.TAG_THUMBNAIL_IMAGE_HEIGHT))
			return null;
		return this._directory
				.getString(ExifDirectory.TAG_THUMBNAIL_IMAGE_HEIGHT)
				+ " pixels";
	}
	...
	private String getFocalPlaneXResolutionDescription()
		...
		return rational.getReciprocal().toSimpleString(
				this._allowDecimalRepresentationOfRationals)
	...
	private String getExifImageWidthDescription() throws MetadataException {
		if (!this._directory.containsTag(ExifDirectory.TAG_EXIF_IMAGE_WIDTH))
			return null;
		return this._directory.getInt(ExifDirectory.TAG_EXIF_IMAGE_WIDTH)
				+ " pixels";
	}
	...
	private String getExifImageHeightDescription() throws MetadataException {
		if (!this._directory.containsTag(ExifDirectory.TAG_EXIF_IMAGE_HEIGHT))
			return null;
		return this._directory.getInt(ExifDirectory.TAG_EXIF_IMAGE_HEIGHT)
				+ " pixels";
	}
	...
	private String getColorSpaceDescription() throws MetadataException {
		if (!this._directory.containsTag(ExifDirectory.TAG_COLOR_SPACE))
			return null;
		int colorSpace = this._directory.getInt(ExifDirectory.TAG_COLOR_SPACE);
		if (colorSpace == 1) {
			return "sRGB";
	...
	private String getFocalLengthDescription() throws MetadataException {
		...
		Rational focalLength = this._directory
	...
	private String getFlashDescription() throws MetadataException {
		...
		default:
			return "Unknown ("
					+ this._directory.getInt(ExifDirectory.TAG_FLASH) + ")";
		}
	}
	...
	private String getWhiteBalanceDescription() throws MetadataException {
		...
		default:
			return "Unknown ("
					+ this._directory.getInt(ExifDirectory.TAG_WHITE_BALANCE)
					+ ")";
		}
	}
	...
	private String getMeteringModeDescription() throws MetadataException {
		...
		int meteringMode = this._directory
				.getInt(ExifDirectory.TAG_METERING_MODE);
		switch (meteringMode) {
		case 0:
			return "Unknown";
		case 1:
	...
	private String getSubjectDistanceDescription() throws MetadataException {
		if (!this._directory.containsTag(ExifDirectory.TAG_SUBJECT_DISTANCE))
			return null;
		Rational distance = this._directory
	...
	private String getCompressionLevelDescription() throws MetadataException {
		if (!this._directory.containsTag(ExifDirectory.TAG_COMPRESSION_LEVEL))
			return null;
		Rational compressionRatio = this._directory
				.getRational(ExifDirectory.TAG_COMPRESSION_LEVEL);
		String ratio = compressionRatio
				.toSimpleString(this._allowDecimalRepresentationOfRationals);
	...
	private String getThumbnailLengthDescription() {
		if (!this._directory.containsTag(ExifDirectory.TAG_THUMBNAIL_LENGTH))
			return null;
		return this._directory.getString(ExifDirectory.TAG_THUMBNAIL_LENGTH)
				+ " bytes";
	}

	private String getThumbnailOffsetDescription() {
		if (!this._directory.containsTag(ExifDirectory.TAG_THUMBNAIL_OFFSET))
			return null;
		return this._directory.getString(ExifDirectory.TAG_THUMBNAIL_OFFSET)
				+ " bytes";
	}

	private String getYResolutionDescription() throws MetadataException {
		if (!this._directory.containsTag(ExifDirectory.TAG_Y_RESOLUTION))
			return null;
		Rational resolution = this._directory
	...
	private String getXResolutionDescription() throws MetadataException {
		if (!this._directory.containsTag(ExifDirectory.TAG_X_RESOLUTION))
			return null;
		Rational resolution = this._directory
	...
	private String getExposureTimeDescription() {
		if (!this._directory.containsTag(ExifDirectory.TAG_EXPOSURE_TIME))
			return null;
		return this._directory.getString(ExifDirectory.TAG_EXPOSURE_TIME)
				+ " sec";
	}
	...
	private String getShutterSpeedDescription() throws MetadataException {
		...
		int apexValue = this._directory.getInt(ExifDirectory.TAG_SHUTTER_SPEED);
	...
	private String getFNumberDescription() throws MetadataException {
		if (!this._directory.containsTag(ExifDirectory.TAG_FNUMBER))
			return null;
		Rational fNumber = this._directory
				.getRational(ExifDirectory.TAG_FNUMBER);
		java.text.DecimalFormat formatter = new DecimalFormat("0.#");
	...
	private String getYCbCrPositioningDescription() throws MetadataException {
		if (!this._directory.containsTag(ExifDirectory.TAG_YCBCR_POSITIONING))
			return null;
		int yCbCrPosition = this._directory
				.getInt(ExifDirectory.TAG_YCBCR_POSITIONING);
	...
	private String getOrientationDescription() throws MetadataException {
		if (!this._directory.containsTag(ExifDirectory.TAG_ORIENTATION))
			return null;
		int orientation = this._directory.getInt(ExifDirectory.TAG_ORIENTATION);
		switch (orientation) {
		case 1:
	...
	private String getResolutionDescription() throws MetadataException {
		if (!this._directory.containsTag(ExifDirectory.TAG_RESOLUTION_UNIT))
			return "";
		// '1' means no-unit, '2' means inch, '3' means centimeter. Default
		// value is '2'(inch)
		int resolutionUnit = this._directory
				.getInt(ExifDirectory.TAG_RESOLUTION_UNIT);
	...
	private String getSensingMethodDescription() throws MetadataException {
		if (!this._directory.containsTag(ExifDirectory.TAG_SENSING_METHOD))
			return null;
		// '1' Not defined, '2' One-chip color area sensor, '3' Two-chip color
		// area sensor
		// '4' Three-chip color area sensor, '5' Color sequential area sensor
		// '7' Trilinear sensor '8' Color sequential linear sensor, 'Other'
		// reserved
		int sensingMethod = this._directory
				.getInt(ExifDirectory.TAG_SENSING_METHOD);
	...
	private String getComponentConfigurationDescription()
			throws MetadataException {
		int[] components = this._directory
	 ...
	 * to a well-known version number, where possible. For example, (hex) 30 32
	 * 31 30 == 2.10).
	 * 
	 * @param components
