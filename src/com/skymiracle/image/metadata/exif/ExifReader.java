...
public class ExifReader implements MetadataReader {
	 ...
	 */
	static final int[] BYTES_PER_FORMAT = { 0, 1, 1, 2, 4, 8, 1, 1, 2, 4, 8, 4,
			8 };
	...
	public Metadata extract(Metadata metadata) {
		...
		// once we know there's some data, create the directory and start
		// working on it
		...
		if (!"Exif\0\0".equals(new String(this._data, 0, 6))) {
	...
	private void processDirectory(Directory directory, int dirStartOffset) {
		...
		// System.err.println("dirTagCount = " + dirTagCount);

		// Handle each tag in this directory
		for (int dirEntry = 0; dirEntry < dirTagCount; dirEntry++) {
			int dirEntryOffset = calculateDirectoryEntryOffset(dirStartOffset,
					dirEntry);
			...
			int tagValueOffset = calculateTagValueOffset(byteCount,
					dirEntryOffset);
			if (tagValueOffset < 0) {
			...
			int subdirOffset = TIFF_HEADER_START_OFFSET
					+ get32Bits(tagValueOffset);

			switch (tagType) {
			case TAG_EXIF_OFFSET:
				processDirectory(this._metadata
						.getDirectory(ExifDirectory.class), subdirOffset);
				continue;
			case TAG_INTEROP_OFFSET:
				processDirectory(this._metadata
						.getDirectory(ExifInteropDirectory.class), subdirOffset);
				continue;
			case TAG_GPS_INFO_OFFSET:
				processDirectory(this._metadata
						.getDirectory(GpsDirectory.class), subdirOffset);
				continue;
			case TAG_MAKER_NOTE:
		...
		// hence
		int nextDirectoryOffset = get32Bits(dirStartOffset + 2 + 12
				* dirTagCount);
		if (nextDirectoryOffset != 0) {
			nextDirectoryOffset += TIFF_HEADER_START_OFFSET;
			if (nextDirectoryOffset >= this._data.length) {
	...
	private void processMakerNote(int subdirOffset) {
		...
		Directory exifDirectory = this._metadata
		...
		if ("OLYMP".equals(new String(this._data, subdirOffset, 5))) {
			...
			if ("Nikon".equals(new String(this._data, subdirOffset, 5))) {
				...
				/*
				 * There are two scenarios here: Type 1: :0000: 4E 69 6B 6F 6E
				 * 00 01 00-05 00 02 00 02 00 06 00 Nikon........... :0010: 00
				 * 00 EC 02 00 00 03 00-03 00 01 00 00 00 06 00 ................
				 * Type 3: :0000: 4E 69 6B 6F 6E 00 02 00-00 00 4D 4D 00 2A 00
				 * 00 Nikon....MM.*... :0010: 00 08 00 1E 00 01 00 07-00 00 00
				 * 04 30 32 30 30 ............0200
				 */
				if (this._data[subdirOffset + 6] == 1) {
					...
					// TODO at this point we're assuming that the MM ordering is
					// continuous with the rest of the file
					// (this seems to be the case, but I don't have many sample
					// images)
					// TODO shouldn't be messing around with this static
					// variable (not threadsafe)
					// instead, should pass an additional offset to the
					// processDirectory method
					int oldHeaderStartOffset = TIFF_HEADER_START_OFFSET;
					TIFF_HEADER_START_OFFSET = subdirOffset + 10;
					processDirectory(this._metadata
							.getDirectory(NikonType3MakernoteDirectory.class),
							subdirOffset + 18);
			...
			// Canon Makernote
			processDirectory(this._metadata
					.getDirectory(CanonMakernoteDirectory.class), subdirOffset);
		} else if ("Casio".equalsIgnoreCase(cameraModel)) {
			// Casio Makernote
			processDirectory(this._metadata
					.getDirectory(CasioMakernoteDirectory.class), subdirOffset);
		} else if ("FUJIFILM".equals(new String(this._data, subdirOffset, 8))
			...
			processDirectory(this._metadata
					.getDirectory(FujiFilmMakernoteDirectory.class), ifdStart);
	...
	private boolean isDirectoryLengthValid(int dirStartOffset) {
		...
		if (dirLength + dirStartOffset + TIFF_HEADER_START_OFFSET >= this._data.length) {
	...
	private void processTag(Directory directory, int tagType,
		...
		case FMT_STRING:
			...
			if (tagType == ExifDirectory.TAG_USER_COMMENT) {
				s = readCommentString(tagValueOffset, componentCount,
						formatCode);
			} else {
		...
		case FMT_BYTE:
			if (componentCount == 1) {
				...
				// fine
				int b = this._data[tagValueOffset];
		...
		case FMT_DOUBLE:
			if (componentCount == 1) {
				int i = this._data[tagValueOffset];
	...
	/*
	 * private void processExifTag(int tagType, int tagValueOffset, int
	 * componentCount, int formatCode) { switch (tagType) { // TODO test this
	 * still works case TAG_USER_COMMENT: // Olympus has this padded with
	 * trailing spaces. Remove these first. // ArrayIndexOutOfBoundsException
	 * bug fixed by Hendrik W?rdehoff - 20 Sep 2002 int byteCount =
	 * componentCount * BYTES_PER_FORMAT[formatCode]; for (int i = byteCount -
	 * 1; i >= 0; i--) { if (_data[tagValueOffset + i] == ' ') {
	 * _data[tagValueOffset + i] = (byte)'\0'; } else { break; } } // Copy the
	 * comment if ("ASCII".equals(new String(_data, tagValueOffset, 5))) { for
	 * (int i = 5; i < 10; i++) { byte b = _data[tagValueOffset + i]; if (b !=
	 * '\0' && b != ' ') { _metadata.setString(DIRECTORY_EXIF_EXIF,
	 * TAG_USER_COMMENT, readString(tagValueOffset + i, 199)); break; } } } else {
	 * _metadata.setString(DIRECTORY_EXIF_EXIF, TAG_USER_COMMENT,
	 * readString(tagValueOffset, 199)); } break;
	 *  // TODO work out what to do with this calculation // More relevant info
	 * always comes earlier, so only use this field if we don't // have
	 * appropriate aperture information yet. case TAG_APERTURE: case
	 * TAG_MAX_APERTURE: if (!_metadata.containsTag(DIRECTORY_EXIF_EXIF,
	 * TAG_FNUMBER)) { _metadata.setFloat(DIRECTORY_EXIF_EXIF, TAG_FNUMBER,
	 * (float)Math.exp(convertTagToNumber(tagValueOffset, formatCode) *
	 * Math.log(2) * 0.5)); } break;
	 *  // TODO copy these comments somewhere // Simplest way of expressing
	 * exposure time, so I trust it most (overwrite previously computd value if
	 * there is one) case TAG_EXPOSURE_TIME: // Indicates the distance the
	 * autofocus camera is focused to. Tends to be less accurate as distance
	 * increases. case TAG_SUBJECT_DISTANCE: // Nice digital cameras actually
	 * save the focal length as a function of how far they are zoomed in. case
	 * TAG_FOCAL_LENGTH: // Simplest way of expressing aperture, so I trust it
	 * the most. (overwrite previously computed value if there is one) case
	 * TAG_FNUMBER: case TAG_EXPOSURE_BIAS:
	 * _metadata.setFloat(DIRECTORY_EXIF_EXIF, tagType,
	 * (float)convertTagToNumber(tagValueOffset, formatCode)); break;
	 *  // TODO work out what to do with this calculation // More complicated
	 * way of expressing exposure time, so only use this value if we don't
	 * already have it from somewhere else. case TAG_SHUTTER_SPEED: if
	 * (!_metadata.containsTag(DIRECTORY_EXIF_EXIF, TAG_EXPOSURE_TIME)) {
	 * _metadata.setFloat(DIRECTORY_EXIF_EXIF, TAG_EXPOSURE_TIME, (float)(1 /
	 * Math.exp(convertTagToNumber(tagValueOffset, formatCode) * Math.log(2)))); }
	 * break; } }
	 */
	...
	private String readString(int offset, int maxLength) {
		int length = 0;
		while ((offset + length) < this._data.length
				&& this._data[offset + length] != '\0' && length < maxLength) {
			length++;
		}
	 ...
	 */
	private String readCommentString(int tagValueOffset, int componentCount,
			int formatCode) {
		// Olympus has this padded with trailing spaces. Remove these first.
		// ArrayIndexOutOfBoundsException bug fixed by Hendrik W?rdehoff - 20
		// Sep 2002
		int byteCount = componentCount * BYTES_PER_FORMAT[formatCode];
		for (int i = byteCount - 1; i >= 0; i--) {
			if (this._data[tagValueOffset + i] == ' ') {
				this._data[tagValueOffset + i] = (byte) '\0';
			} else {
				break;
			}
		}
	...
	private int get16Bits(int offset) {
		...
		if (this._isMotorollaByteOrder) {
			// Motorola big first
			return (this._data[offset] << 8 & 0xFF00)
					| (this._data[offset + 1] & 0xFF);
		} else {
			// Intel ordering
			return (this._data[offset + 1] << 8 & 0xFF00)
					| (this._data[offset] & 0xFF);
		}
	}
	...
	private int get32Bits(int offset) {
		...
		if (this._isMotorollaByteOrder) {
			// Motorola big first
			return (this._data[offset] << 24 & 0xFF000000)
					| (this._data[offset + 1] << 16 & 0xFF0000)
					| (this._data[offset + 2] << 8 & 0xFF00)
					| (this._data[offset + 3] & 0xFF);
		} else {
			// Intel ordering
			return (this._data[offset + 3] << 24 & 0xFF000000)
					| (this._data[offset + 2] << 16 & 0xFF0000)
					| (this._data[offset + 1] << 8 & 0xFF00)
					| (this._data[offset] & 0xFF);
		}
	}
}
