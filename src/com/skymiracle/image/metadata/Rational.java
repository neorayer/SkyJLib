...
public class Rational extends java.lang.Number implements Serializable {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -4753245333989998641L;
	 ...
	 * This may involve rounding or truncation. This implementation simply casts
	 * the result of <code>doubleValue()</code> to <code>short</code>.
	 * 
	 * @return the numeric value represented by this object after conversion to
	...
	/**
	 * Checks if this rational number is an Integer, either positive or
	 * negative.
	 */
	...
	public String toSimpleString(boolean allowDecimal) {
		if (this.denominator == 0 && this.numerator != 0) {
			...
			return Integer.toString(intValue());
				...
				&& this.denominator % this.numerator == 0) {
			...
			if (allowDecimal) {
				String doubleString = Double.toString(simplifiedInstance
						.doubleValue());
	...
	/**
	 * Decides whether a brute-force simplification calculation should be
	 * avoided by comparing the maximum number of possible calculations with
	 * some threshold.
	 * 
	 * @return true if the simplification should be performed, otherwise false
	 */
	private boolean tooComplexForSimplification() {
		double maxPossibleCalculations = (((Math.min(this.denominator,
				this.numerator) - 1) / 5d) + 2);
		return maxPossibleCalculations > this.maxSimplificationCalculations;
	}

	/**
	 * Compares two <code>Rational</code> instances, returning true if they
	 * are mathematically equivalent.
	 * 
	 * @param obj
	 *            the Rational to compare this instance to.
	 * @return true if instances are mathematically equivalent, otherwise false.
	 *         Will also return false if <code>obj</code> is not an instance
	 *         of <code>Rational</code>.
	 */
	@Override
	 ...
	 * </pre></code>
	 * 
	 * @return a simplified instance, or if the Rational could not be
	 *         simpliffied, returns itself (unchanged)
	 */
	public Rational getSimplifiedInstance() {
		if (tooComplexForSimplification()) {
			return this;
		}
		for (int factor = 2; factor <= Math.min(this.denominator,
