...
public class Getopt extends Object {
	...
	public Getopt(String progname, String[] argv, String optstring,
		...
		if (System.getProperty("gnu.posixly_correct", null) == null)
			...
			this._messages = ResourceBundle.getBundle(
	...
	protected int checkLongOption() {
		...
		for (int i = 0; i < this.long_options.length; i++) {
			if (this.long_options[i].getName().startsWith(
					this.nextchar.substring(0, nameend))) {
				if (this.long_options[i].getName().equals(
						this.nextchar.substring(0, nameend))) {
					// Exact match found
		...
		if (ambig && !exact) {
			if (this.opterr) {
				Object[] msgArgs = { this.progname, this.argv[this.optind] };
				System.err.print(MessageFormat.format(this._messages
				...
				if (pfound.has_arg != LongOpt.NO_ARGUMENT) {
					...
					if (this.opterr) {
						...
						if (this.argv[this.optind - 1].startsWith("--")) {
							Object[] msgArgs = { this.progname, pfound.name };
						...
						// +option or -option
							...
							Object[] msgArgs = {
									this.progname,
									new Character(this.argv[this.optind - 1]
											.charAt(0)).toString(), pfound.name };
							System.err.print(MessageFormat.format(
									this._messages
	...
	public int getopt() {
		...
		if ((this.nextchar == null) || (this.nextchar.equals(""))) {
			...
			if (this.ordering == PERMUTE) {
				...
				while ((this.optind < this.argv.length)
						&& (this.argv[this.optind].equals("")
								|| (this.argv[this.optind].charAt(0) != '-') || this.argv[this.optind]
								.equals("-"))) {
					this.optind++;
				}
			...
			if (this.argv[this.optind].equals("")
					|| (this.argv[this.optind].charAt(0) != '-')
					|| this.argv[this.optind].equals("-")) {
		 ...
		 */
		if ((this.long_options != null)
				&& (this.argv[this.optind].startsWith("--") || (this.long_only && ((this.argv[this.optind]
						.length() > 2) || (this.optstring
						.indexOf(this.argv[this.optind].charAt(1)) == -1))))) {
			...
			if (!this.long_only || this.argv[this.optind].startsWith("--")
				...
				if (this.opterr) {
					if (this.argv[this.optind].startsWith("--")) {
						Object[] msgArgs = { this.progname, this.nextchar };
						System.err.print(MessageFormat.format(this._messages
						...
						Object[] msgArgs = {
								this.progname,
								new Character(this.argv[this.optind].charAt(0))
										.toString(), this.nextchar };
						System.err.print(MessageFormat.format(this._messages
		...
		int c = this.nextchar.charAt(0); // **** Do we need to check for
											// empty str?
